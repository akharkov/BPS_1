...
/* 
    ...
    try{
            ...
            try{
                ...
                if (rs){
                    ...
                    try{
                        ...
                        result =  await pool.request().query(sqlExpr)
                                ...
                                case 0: // ССМ  ранее не регистрировалась
                                    ...
                                    try{
                                        ...
                                        if (rs) {
                                            ...
                                            await this.addDepositAndList(pool, curFileName, objTmp2 , Office = `0000`, tmp_rel_id_CashMachine);
                                    
                                        };
                                    }
                                    catch(err){
                                        other.myConsoleLog(`${ new Date().toLocaleString()} : Ошибочка 1 записи в DB ${sqlExpr}`,err)
                                        db.dbLogAdd(pool,`ErrorLoadDeposit1`,`fparser.fcontent File ${curFileName}`,`${err} --- command=${sqlExpr.replace(/'/g,"~")}  `,`convert(datetime,'${new Date().toISOString()}')`); 
                                        CashMachAdded = false;
                                    } ;
                                    break;
                        ...
                        other.myConsoleLog(`${ new Date().toLocaleString()} : Ошибочка запроса в DB ${sqlExpr}`, err)
                        db.dbLogAdd(pool,`ErrorLoadDeposit2`,`fparser.fcontent`,`err=${err} command=${sqlExpr}`,`convert(datetime,'${new Date().toISOString()}')`); 
                ...
                other.myConsoleLog(`${ new Date().toLocaleString()} : Ошибочка 2 записи в DB ${sqlExpr}`, err)
                db.dbLogAdd(pool,`Error load transport file`,`fparser.fcontent`,`transpFileName=${curFileName} ${err}`,`convert(datetime,'${new Date().toISOString()}')`); 
        ...
        other.myConsoleLog(`${ new Date().toLocaleString()} : Ошибочка 3 записи в DB ${sqlExpr} `, err);
...
exports.addDepositAndList =  async function addDepositAndList(fp_pool, fp_curFileName, fp_objTmp2 , fp_Office = `0000`, fp_tmp_rel_id_CashMachine){
let res;
   ...
   /*  if (CashMachAdded ) { */ //ССМ присутствует в справочнике
    try{                        
        ...
        try{
            res = await fp_pool.request().query(sqlExpr);
            if(res) {
                //other.myConsoleLog(`Запись в DB ${sqlExpr}`)
                ...
                fp_objTmp2.descriptBN.forEach(element => {
                    ...
                    .catch(res=>{
                        console.log(`${ new Date().toLocaleString()} : Ошибочка записи в DB №№ купюр ${sqlExpr}`, res)
                        db.dbLogAdd(fp_pool,`ErrorLoadDeposit`,`fparser.fcontent`,`Ошибочка записи в DB №№ купюр ${sqlExpr} res=${res}`,`convert(datetime,'${new Date().toISOString()}')`); 
                    ...
                    /* try{
                       ...
                       res = (async (res)=>{
                        ...
                        console.log(`${ new Date().toLocaleString()} : Ошибочка записи в DB №№ купюр ${sqlExpr}`, res)
                        db.dbLogAdd(pool,`ErrorLoadDeposit`,`fparser.fcontent`,`Ошибочка записи в DB №№ купюр ${sqlExpr} res=${res}`,`convert(datetime,'${new Date().toISOString()}')`); 
            ...
            db.dbLogAdd(fp_pool,`ErrorLoadDeposit5`,`fparser.fcontent`,`err=${err} command=${sqlExpr}`,`convert(datetime,'${new Date().toISOString()}')`); 
        
        } ; 
    }
    catch(err){
        other.myConsoleLog(`${ new Date().toLocaleString()} : Ошибочка в модуле  addDepositAndList `, err)
        db.dbLogAdd(pool,`ErrorLoadDeposit6`,`fparser.fcontent`,`Ошибочка в модуле  addDepositAndList ${err}`,`convert(datetime,'${new Date().toISOString()}')`); 
